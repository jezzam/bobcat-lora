...
## WiFi/TCP companion-app support added — hybrid repeater + phone chat (2026-07-29)

Goal: keep the device as a full, always-relaying repeater for the wider mesh, while also letting the official MeshCore phone app connect directly to it over the LAN (WiFi/TCP) for personal chat — no separate companion radio needed at home. The app just needs an IP:port; this is MeshCore's standard companion protocol (the same one `SerialWifiInterface` implements on ESP32 boards), not anything app-specific.

**Architecture**: MeshCore has two entirely separate app roles — `simple_repeater` (`mesh::Mesh` + `CommonCLICallbacks`, plain-text CLI only, what was originally deployed) and `companion_radio` (`BaseChatMesh`, full contacts/chat/channels, binary companion protocol over BLE/WiFi). Switched the build from the former to the latter, extended with:
- **`LinuxTcpInterface`** (new, `variants/linux_udp_bridge/LinuxTcpInterface.h/.cpp`) — POSIX-socket port of `SerialWifiInterface`'s frame protocol (3-byte header, single active client), listening on port 5000 (`MESHCORE_TCP_PORT` env var).
- **A small custom plain-text CLI** (`MyMesh::handleCLICommand`, ~60 lines in `examples/companion_radio/MyMesh.cpp`) added alongside the companion protocol, rather than porting `CommonCLI` itself — `CommonCLI` needs `RegionMap`/`ClientACL` (region-switching, multi-client ACLs), neither used by this project. Reuses the same `StatsFormatHelper` real `simple_repeater` used, so `stats-packets`/`stats-radio` JSON output is byte-identical to what the existing SSH tooling/monitor scripts already parse. Covers: `stats-packets`, `stats-radio`, `stats-core`, `neighbors`, `discover.neighbors` (calls `advert()` — confirmed to send the same zero-hop self-advert the old CLI's `discover.neighbors` triggered), `get`/`set radio`, `get`/`set name`, `set lat`/`lon`, `reboot`.
- **`client_repeat` defaulted to `1`** in the constructor (was `0` — companion_radio assumes a battery/portable "personal node that optionally helps out," not a dedicated infrastructure repeater). `allowPacketForward()` (`src/Mesh.cpp:14`) is the same core relay-gate mechanism either role uses, so this makes relay behavior unconditional, matching the old role — confirmed no capability lost by switching.
- Both interfaces (stdin CLI, TCP companion protocol) run concurrently against the same `MyMesh`/identity in a new `linux_companion_main.cpp`.

**Build fixes needed** (`examples/companion_radio/`, `src/helpers/BaseChatMesh.cpp` — all additive, same pattern as the original 3-file port):
- `MAX_GROUP_CHANNELS` has no compiled-in default anywhere (every real board's `platformio.ini` sets it) — added `=40` to CMakeLists (matches common board values).
- `checkCLIRescueCmd()` — a hardware rescue console (BLE-pin reset, filesystem browsing via ESP32/NRF52-only `File::openNextFile()`/`isDirectory()`/`name()`/`size()`) triggered by a physical button combo on boot. Nothing calls `enterCLIRescue()` in this build, so it's dead code — stubbed to a no-op for `LINUX_PLATFORM` rather than porting directory-browsing methods our `compat/FS.h` was never meant to have.
- `<base64.hpp>` (`densaugeo/base64`, an external PlatformIO lib, only used by `BaseChatMesh::addChannel()` for decoding a group-channel PSK) — wrote a tiny `compat/base64.hpp` wrapping libsodium's `sodium_base642bin()` (already a build dependency, same pattern `PktFwdRadio.cpp` uses for LoRa payload encode/decode) instead of vendoring a whole separate library for one function. Capped the decode length at 32 bytes (`PUB_KEY_SIZE`, the real destination buffer size) rather than trusting the caller-supplied input length, which the original library's API contract doesn't protect against.

**Critical operational finding — NodePrefs schema is genuinely incompatible between the two roles**: `examples/companion_radio/NodePrefs.h` and `src/helpers/CommonCLI.h`'s `NodePrefs` are different struct layouts, both persisted via explicit field-by-field `file.read()`/`file.write()` in fixed order — not a raw struct dump, but still schema-specific. Backed up `/update/meshcore-data` before cutover (`meshcore-data.pre-hybrid-backup-<timestamp>`) and treated the switch as a fresh prefs state, confirmed correct: identity (`IdentityStore` is identical/shared between both roles — same public key `029AF8DDDAE28F42A349D91E33D9C04E911F1526843BE24DEF36A48C6545EE4B` carried over with zero special handling) and compile-time radio defaults were already correct after the switch; only `node_name`/`lat`/`lon` needed re-applying via the new CLI (`set name`/`set lat`/`set lon`), confirmed persisting correctly across a `docker restart`.

**Deployment note — this device has an aggressive image auto-pruner**: a built-but-not-yet-running Docker image got silently removed twice during this session, within roughly 10-20 seconds of the build finishing (`docker build` succeeds, next command sees "Unable to find image locally"). Root cause not identified (not `docker image prune` alone — that showed `reclaimed=0` in `docker events` at the time). Workaround: chain `docker build && docker run` in a single command/session rather than building and running as separate steps.

**Verification done**: clean build, CLI parity confirmed (`stats-packets`/`stats-radio`/`neighbors`/`get radio` all match pre-switch format and values), TCP:5000 confirmed listening (`ss -tlnp`) and reachable both on-device and from another LAN host, identity and re-applied settings both confirmed surviving a `docker restart`. **Not yet done**: connecting the actual MeshCore phone app to `<device-ip>:5000` — needs a physical phone with the app, first real end-to-end validation of this feature.
